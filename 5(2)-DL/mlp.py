...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        z3 = h2 @ W3 + b3
        h3 = tanh(z3 + h1)  # residual connection
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        # h3 = tanh(z3 + h1)의 gradient
        dh3 = dz3 * (1 - h3**2)  # tanh derivative
        db3 = np.sum(dh3, axis=0)
        dW3 = z2.T @ dh3 + 2 * L2_norm * W3
        dz2 = dh3 @ W3.T
        
        # residual connection 때문에 h1에도 gradient가 전파됨
        dh1_residual = dh3 * (1 - h3**2)  # tanh derivative for residual part
        
        # h2 = leakyrelu(z2)의 gradient
        dh2 = dz2 * np.where(z2 > 0, 1, 0.01)  # leakyrelu derivative
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T

        # h1 = relu(z1)의 gradient + residual connection의 gradient
        dh1 = dz1 * (z1 > 0) + dh1_residual  # relu derivative + residual
